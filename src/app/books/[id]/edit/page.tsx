...
export default function EditBookPage() {
  ...
  const [existingEbookFile, setExistingEbookFile] = useState('')
  ...
  useEffect(() => {
    ...
    fetch(`/api/books/${params.id}`).then((r) => r.json()).then((d) => {
      if (d.book) {
        setForm({
          title: d.book.title, author: d.book.author, publisher: d.book.publisher || '',
          year: d.book.year?.toString() || '', categoryId: d.book.categoryId.toString(),
          stock: d.book.stock.toString(), description: d.book.description || '',
        })
        if (d.book.coverImage) setExistingCover(d.book.coverImage)
        if (d.book.isEbook) { setIsEbook(true); setExistingEbookFile(d.book.ebookFile || '') }
      }
    }).catch(() => {})
  }, [params.id])
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    let ebookPath = existingEbookFile
    ...
    try {
      ...
      const body: Record<string, unknown> = { ...form, coverImage }
      if (isEbook) { body.isEbook = true; body.ebookFile = ebookPath }
      else { body.isEbook = false; body.ebookFile = null }
  ...
  return (
    <div className="relative w-full z-10">
      ...
      <div className="absolute top-0 left-1/2 -translate-x-1/2 z-[-1] pointer-events-none w-full text-center mt-[-60px]">
         <h1 className="mc-ghost-watermark select-none text-[150px] md:text-[250px]">EDIT BOOK</h1>
      </div>
      ...
      <div className="mc-card-stadium p-6 md:p-12 mb-10 mt-10 max-w-4xl mx-auto shadow-md">
        <div className="mb-10 text-center">
          ...
          <h1 className="mc-heading-2 text-[var(--color-ink)]">Edit Buku</h1>
          <p className="text-[16px] font-[450] text-[var(--color-slate)] mt-4 max-w-lg mx-auto">
            Perbarui data buku yang sudah ada di perpustakaan.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                ...
                <div>
                  ...
                  {(coverPreview || existingCover) && (
                    <div className="bg-white p-2 rounded-[16px] inline-block shadow-sm border border-black/5">
                      <Image src={coverPreview || existingCover} alt="Pratinjau Sampul" width={100} height={144} className="h-32 w-auto object-cover rounded-[8px]" />
                    </div>
                  )}
                </div>
              </div>
            ...
            {isEbook && (
              <div className="col-span-1 md:col-span-2 ml-0 sm:ml-10">
                ...
                {existingEbookFile && !ebookFile && (
                  <p className="flex items-center gap-2 text-[14px] text-[var(--color-slate)] mb-3 bg-white/40 p-3 rounded-[16px] sm:rounded-full border border-black/5 inline-flex pr-6">
                    <svg className="w-4 h-4 ml-2 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                    File sudah ada. Pilih PDF baru untuk mengganti.
                  </p>
                )}
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 pt-8 mt-8 border-t border-[var(--color-ink)]/5">
            <button type="submit" disabled={loading}
              ...
              {uploadStatus || (loading ? 'Menyimpan...' : 'Simpan Perubahan')}
            </button>
