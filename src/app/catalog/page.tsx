...
export default function CatalogPage() {
  ...
  async function handleBorrow(bookId: number) {
    ...
    try {
      ...
      const data = await res.json()
      if (!res.ok) {
        setMessage({ type: 'error', text: data.error || 'Gagal meminjam' })
      } else {
        setMessage({ type: 'success', text: `"${data.transaction.book.title}" berhasil dipinjam!` })
        fetchBooks()
      }
    } catch {
          ...
          {books.map((book) => (
            <div key={book.id} className="bg-white rounded-xl shadow-sm border p-5 hover:shadow-md transition">
              ...
              <div className="mt-3 flex items-center justify-between">
                ...
                {book.stock > 0 && user && (
                  <button onClick={() => handleBorrow(book.id)} disabled={borrowingId === book.id}
                    className="text-xs bg-primary text-white px-3 py-1.5 rounded-lg hover:bg-primary-dark transition disabled:opacity-50">
                    {borrowingId === book.id ? 'Memproses...' : 'Pinjam'}
                  </button>
                )}
                {book.stock > 0 && !user && (
                  <Link href="/login" className="text-xs text-primary hover:underline">Masuk untuk meminjam</Link>
                )}
              </div>
            </div>
          ))}
